...
""" Purpose of this module is to identify the types of fields
    Classes include:
      ...
      get_field_type()
      get_type()
      DateTimeChecker() - determines if arg is a datetime and which format
    Todo:
      - FIX PARSING ISSUES WITH TAB-DELIMITED FILES
      - ADD TESTING FOR FIELDTYPER
      - quartiles
      - consistency metric
      ...
      - add date-type determination
      - change returned data format to be based on field
      - add unittesting for dates - and add more date formats
"""
...
def get_max(type, values):
    """ Returns the maximum value found for input dictionary or list
        ...
        Test Coverage:
          - complete via test harness
    """
    assert(type in ['integer', 'float', 'string', None])
...
class DatetimeChecker(object):
    """Return (True/False, <scope>) for the given datetime string.
       >>> _datetime_check("2009")
       (True, 'year')
       >>> _datetime_check("2009-12")
       (True, 'month')
       >>> _datetime_check("2009-12-25")
       (True, 'day')
       >>> _datetime_check("2009-12-25 13")
       (True, 'hour')
       >>> _datetime_check("2009-12-25 13:05")
       (True, 'minute')
       >>> _datetime_check("2009-12-25 13:05:14")
       (True, 'second')
       >>> _datetime_check("2009-12-25 13:05:14.453728")
       (True, 'microsecond')
       Code originally came from activestate recipe site:
    ...
    def datetime_check(self, time_str):
        """ Determine if arg is a datetime and if so what format
  
            Args:
               time_str - character string that may be a date
            Returns:
               status   - True if date/time False if not
               scope    - kind of datetime
        """
        for scope, pattern, format in self.formats:
