...
class GristleMetaData(object):
    ...
    def __init__(self, db_dir=None, db_name='metadata2.db'):
    ...
    def create_db_objects(self):
        ...
        metadata = MetaData(bind=self.db, reflect=True)
        print metadata
        schema_table      = metadata.tables['schema']
        struct_type_table = metadata.tables['struct_type']
        struct_table      = metadata.tables['struct']

        print 'Metadata testing: '
        #s      = metadata.tables['struct'].select()
        s      = struct_table.select()
        result = s.execute()
    ...
    def create_db(self):
        ...
        self.struct_type = Table('struct_type',
                         metadata,
